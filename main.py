...
def resume_train_utils(resume_path, begin_epoch, optimizer, scheduler):
    ...
    if optimizer is not None and 'optimizer' in checkpoint and False:
...
def get_train_utils(opt, model_parameters):
    ...
    alls=[0,1]
    alls.remove(opt.val_fold)
    #alls.remove(opt.inf_fold)
    train_data = get_training_data(opt.trainlist, opt.testlist,
                                   opt.dataset, opt.input_type, opt.file_type,alls,
...
def get_val_utils(opt):
    ...
    val_data = get_validation_data(opt.testlist,
                                               None, opt.dataset,
...
def get_inference_utils(opt):
    ...
    inference_data = get_inference_data(
        opt.testlist, None, opt.dataset, opt.input_type,
        opt.file_type, opt.inf_fold,opt.inference_subset, spatial_transform,
        temporal_transform)
